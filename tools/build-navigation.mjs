...
const navFile = path.join(root, "apostilas-nav.js");
const scriptTag = '<script src="./apostilas-nav.js" defer></script>';
...
const preferredOrder = [
  ...
  "guia-completo-modifiers-blender-5x.html",
  "guia-bolso-hard-surface-blender-5x.html",
  "apostila-rigging-blender-5x.html",
];
...
const cleanTitle = (html, file) => {
  const title = html.match(/<title>([\s\S]*?)<\/title>/i)?.[1];
  if (!title) return file.replace(/\.html$/i, "").replace(/[-_]+/g, " ");
...
for (const file of files) {
  ...
  pages.push({ file, title: cleanTitle(html, file) });

  if (!html.includes("apostilas-nav.js")) {
    html = html.replace(/<\/body>\s*<\/html>\s*$/i, `${scriptTag}\n</body>\n</html>\n`);
    await writeFile(fullPath, html);
  }
}

const navSource = `// Gerado por tools/build-navigation.mjs. Edite o pipeline, nao esta lista.
(function () {
  "use strict";
...
await writeFile(navFile, navSource);
console.log(`Navegacao gerada para ${pages.length} apostilas.`);
